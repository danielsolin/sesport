...
public sealed class ActivityParticipantFindResultModel(
...
) : PageModel
{
   ...
   )
   {
      if(string.IsNullOrWhiteSpace(Activity.Title))
      {
         ...
         {
            error = "Title is required before finding results."
         });
      }
      ...
      if(Activity.Id is null)
      {
         return BadRequest(new
         {
            error = "Save the activity before queueing a result job."
         });
      }
      ...
      var runId = await editService.QueueFindParticipantsResultAsync(
         savedActivity,
         cancellationToken
      );
