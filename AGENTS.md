...
2. Start PostgreSQL: `docker compose up -d` (run in WSL if Docker is only available there)
3. Run database migrations:
   - PowerShell: `.\database\run-migrations.ps1` (run in WSL if Docker is only available there)
   - Bash: `./database/run-migrations.sh` (run in WSL if Docker is only available there)
...
## Import Tools
Several console applications are available in the `tools` directory for data import:
...
- Database connection string defaults to Host=localhost;Port=5432;Database=sesport;Username=sesport;Password=sesport
- The web app uses Npgsql for PostgreSQL data access
- Ensure PostgreSQL is running and migrated before running the web app or import tools (PostgreSQL must be started via Docker in WSL if Docker is only available there)
